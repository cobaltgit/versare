...
class Sniper(commands.Cog):
    ...
    async def on_message_delete(self, message):
        async with self.bot.snipe_cxn.cursor() as cur:
            ...
            if (optout_uids is not None and message.author.id in optout_uids) or message.author.bot or any(re.search(regex, message.content) for regex in self.SENSITIVE_REGEXPS):
    ...
    async def on_message_edit(self, before, after):
        async with self.bot.snipe_cxn.cursor() as cur:
            ...
            optout_uids = await cur.fetchone()
            if (optout_uids is not None and before.author.id in optout_uids) or before.author.bot or any(re.search(regex, before.content) for regex in self.SENSITIVE_REGEXPS):
                return await cur.close()
